...
var wobbleTSpeed = 8, wobbleTDist = 0.07;

Trophy.prototype.act = function(step) {
  this.wobble += step * wobbleTSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleTDist;
...
Enemy.prototype.act = function(step) {
  ...
  var wobbleEPos = Math.sin(this.wobble) * wobbleEDist;
  this.pos = this.basePos.plus(new Vector(wobbleEPos, 0));
};

var wobbleASpeed = 2, wobbleADist = 4;

Enemy2.prototype.act = function(step) {
  this.wobble += step * wobbleASpeed;
  var wobbleAPos = Math.sin(this.wobble) * wobbleADist;
  this.pos = this.basePos.plus(new Vector(0, wobbleAPos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "enemy2" && this.status == null){
    this.status = "lost";
    this.finishDelay = 1;
  }
  ...
  if (type == "lava" && this.status == null) {
    ...
    coins++;
    document.getElementById("coin").innerHTMl = "Coins Collected: " + coins;
    } else if (type == "trophy") {
      this.actors = this.actors.filter(function(other){
        return other != actor;
      });
    ...
    if (!this.actors.some(function(actor) {
      ...
      if(this.status ="won")
        return document.write("Congratulations You Won!");
    }
  };
